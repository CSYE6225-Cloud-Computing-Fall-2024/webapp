...
//                        .body("Bad Request: Unsupported file type. Only PNG, JPG, and JPEG files are allowed.");
       ...
       if(!imageExists){
           ...
           return ResponseEntity.status(HttpStatus.BAD_REQUEST).cacheControl(CacheControl.noCache()).body("Profile pic already exists for User: " + user.getEmail());
       }
...
//                        .body("Bad Request: Unsupported file type. Only PNG, JPG, and JPEG files are allowed.");
            ...
            if(imageExists){
//                FileEntity fileEntity  = fileService.getImageDetailsByUserID(user.getId());
//                FileUploadRequest fileUploadRequest = new FileUploadRequest(file);
                return ResponseEntity.ok(fileService.getImageDetailsByUserID(user.getId()));
            }else{
                return ResponseEntity.status(HttpStatus.NOT_FOUND).cacheControl(CacheControl.noCache()).body("Profile pic doesn't exists for User: " + user.getEmail());
            }
