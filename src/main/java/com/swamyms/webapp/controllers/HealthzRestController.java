...
public class HealthzRestController {
    ...
    @GetMapping("/cicd")
    private ResponseEntity<?> getCICDStatus(@RequestParam(required = false) HashMap<String, String> params, // Check for query parameters
                                               @RequestBody(required = false) String requestBody // Check for request body
    ) {
        // Prepare headers
        HttpHeaders headers = new HttpHeaders();
    ...
    @PostMapping("/cicd")
    private ResponseEntity<String> handlePostCICDStatus(){
        HttpHeaders headers = new HttpHeaders();
        headers.setCacheControl("no-cache, no-store, must-revalidate");
        headers.add("Pragma", "no-cache");
        // Log the method call
    ...
    @DeleteMapping("/cicd")
    private ResponseEntity<String> handleDeleteCICDStatus(){
        HttpHeaders headers = new HttpHeaders();
        headers.setCacheControl("no-cache, no-store, must-revalidate");
        headers.add("Pragma", "no-cache");
        logger.error("Delete /healthz called, returning METHOD_NOT_ALLOWED.");
    ...
    @PutMapping("/cicd")
    private ResponseEntity<String> handlePutCICDStatus(){
        HttpHeaders headers = new HttpHeaders();
        headers.setCacheControl("no-cache, no-store, must-revalidate");
        headers.add("Pragma", "no-cache");
        logger.error("Put /healthz called, returning METHOD_NOT_ALLOWED.");
    ...
    @PatchMapping("/cicd")
    private ResponseEntity<String> handlePatchCICDStatus(){
        HttpHeaders headers = new HttpHeaders();
        headers.setCacheControl("no-cache, no-store, must-revalidate");
        headers.add("Pragma", "no-cache");
        logger.error("Patch /healthz called, returning METHOD_NOT_ALLOWED.");
    ...
    @RequestMapping(value = "/cicd",method = RequestMethod.HEAD)
    private ResponseEntity<String>  handleHeadCICDStatus(){
        HttpHeaders headers = new HttpHeaders();
        headers.setCacheControl("no-cache, no-store, must-revalidate");
        headers.add("Pragma", "no-cache");
        logger.error("HEAD /healthz called, returning METHOD_NOT_ALLOWED.");
    ...
    @RequestMapping(value = "/cicd",method = RequestMethod.OPTIONS)
    private ResponseEntity<String>  handleOptionsCICDStatus(){
        HttpHeaders headers = new HttpHeaders();
        headers.setCacheControl("no-cache, no-store, must-revalidate");
        headers.add("Pragma", "no-cache");
        logger.error("OPTIONS /healthz called, returning METHOD_NOT_ALLOWED.");
